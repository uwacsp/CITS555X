...
function getData($id, $idname,$idmeasure){
    ...
    global $curl,$authHeader;
    curl_setopt_array($curl, array(
      CURLOPT_URL => "https://i.digitaltwin.ezone.uwa.edu.au//api/trendableData/get?id=$id&from=now-120_minutes&to=now",
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
...
function totaleid($arraydata, $eid){
    ...
    foreach($eid as $newid){
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://i.digitaltwin.ezone.uwa.edu.au//api/trendableData/get?id=$newid&from=now-120_minutes&to=now",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => '',
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
    ...
    foreach($arraydata as &$sendString){
        $sql_checkquery = "SELECT value FROM ems_transaction_ext_table WHERE timestamp = '$sendString[timestamp]' AND attribute_name_ext = '$sendString[attribute_name_ext]'";    
		    ...
		    if($stmt->rowCount() > 0){    
		        ...
		        print("Entry at $sendString[timestamp] already exists<br />");    
            } 
            else {
                #entry does not exist
                print("Entry $sendString[timestamp] does not exist, creating...<br />");
...
$airMeasure = "kg/m^3";
print("Getting airdens<br />");
$airArray = getData($airdensitysid,$airName,$airMeasure);
print("Sending airdensity<br />");
